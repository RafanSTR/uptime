...
// Check if cleanup should run (every 24 hours)
async function checkIfCleanupNeeded(supabase) {
  try {
    // Check last cleanup from webhook logs
    const { data: lastCleanup, error } = await supabase.from('webhook_logs').select('webhook_data, received_at').not('webhook_data->cleanup_result', 'is', null).order('received_at', {
      ascending: false
    }).limit(1).single();
...
async function runCleanup(supabase) {
  try {
    ...
    try {
      ...
      const { count: statusCheckCount, error: countError } = await supabase.from('status_checks').select('*', {
        count: 'exact',
        head: true
      ...
      if (results.statusChecksDeleted > 0) {
        ...
        const { error: deleteError } = await supabase.from('status_checks').delete().lt('checked_at', ninetyDaysAgo.toISOString());
        if (deleteError) {
    ...
    try {
      ...
      // Count records to be deleted
      const { count: webhookCount, error: countError } = await supabase.from('webhook_logs').select('*', {
        count: 'exact',
        head: true
      ...
      if (results.webhookLogsDeleted > 0) {
        ...
        const { error: deleteError } = await supabase.from('webhook_logs').delete().lt('received_at', thirtyDaysAgo.toISOString());
        if (deleteError) {
